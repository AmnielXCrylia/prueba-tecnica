...
class SubMenu extends Conection
{
    ...
    public function save()
    {
        ...
        if(empty($this->id_menu_hijo)) {
            ...
            VALUES (:id_menu_padre, :descripcion, :nombre)");
            $prepare->execute($params);
        } else {
            $params[":id_menu_hijo"] = $this->id_menu_hijo;
            $prepare = $this->prepare("UPDATE menu_hijo SET id_menu_padre = :id_menu_padre,  nombre = :nombre, descripcion = :descripcion WHERE id_menu_hijo = :id_menu_hijo");
            $prepare->execute($params);
        }
    }
